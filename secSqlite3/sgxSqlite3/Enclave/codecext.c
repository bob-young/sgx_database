...
struct timeval start_enc,stop_enc;
struct timeval start_dec,stop_dec;
struct calltime enc_call_time;
struct calltime dec_call_time;
...
void* sqlite3Codec(void* pCodecArg, void* data, Pgno nPageNum, int nMode)
{
  ...
  switch(nMode)
  {
    ...
    case 3: /* Load a page */
      ...
      {
        //start_timing(start_dec);
        #ifdef SGX_ENC_DEC_TIME_TEST
        gettimeofday (&start_dec, NULL);
        #endif

        #if CODEC_TYPE == CODEC_TYPE_GCM128
        CodecDecrypt(codec, nPageNum, (unsigned char*) data, pageSize, tag);
        #else
        CodecDecrypt(codec, nPageNum, (unsigned char*) data, pageSize);
        #endif

        #ifdef SGX_ENC_DEC_TIME_TEST
        //time = stop_timing(start_dec,stop_dec);
        gettimeofday (&stop_dec, NULL);
        //time = (double) ((stop_dec.tv_sec - start_dec.tv_sec)*1000000 + (stop_dec.tv_usec - start_dec.tv_usec)) ;
        time = (double) ((stop_dec.tv_sec - start_dec.tv_sec) + (stop_dec.tv_usec - start_dec.tv_usec)/(double)1000000) ;
        dec_call_time.no_pages++;
        dec_call_time.exec_time+=time;
        //fprintf(stdout,"dec_time:%lfs\n", dec_call_time.exec_time);
    ...
    case 6: /* Encrypt a page for the main database file */
      if (CodecHasWriteKey(codec))
      {
        ...
        #if CODEC_TYPE == CODEC_TYPE_GCM128
        CodecEncrypt(codec, nPageNum, (unsigned char*) data, pageSize, 1, tag);
        #else
        CodecEncrypt(codec, nPageNum, (unsigned char*) data, pageSize, 1);
        #endif

        #ifdef SGX_ENC_DEC_TIME_TEST
        gettimeofday (&stop_enc, NULL);        
        enc_call_time.no_pages++;

        time = (double) ((stop_enc.tv_sec - start_enc.tv_sec) + (stop_enc.tv_usec - start_enc.tv_usec)/(double)1000000) ;
        enc_call_time.exec_time+=time;
        //fprintf(stdout,"enc_time:%lfs\n", enc_call_time.exec_time);
    ...
    case 7: /* Encrypt a page for the journal file */
      ...
      */
      if (CodecHasReadKey(codec))
      {
        ...
        #endif
        #if CODEC_TYPE == CODEC_TYPE_GCM128
        CodecEncrypt(codec, nPageNum, (unsigned char*) data, pageSize, 0, tag);
        #else
